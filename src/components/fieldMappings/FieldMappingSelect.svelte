<script>
  ...
  const handleAddMapping = (_) => {
    if (labelFieldToMap) {
      $fieldMappings[$appSettings.labelType][labelFieldToMap] =
        mappedDatasetField;
      labelFieldToMap = "";
      mappedDatasetField = "";
...
<div class="flex h-[2em] items-center gap-2">
  <select
    class="{!mappedDatasetField ? 'text-[gray]' : ''} m-0 ml-[1em]"
    bind:value={mappedDatasetField}
  >
  ...
  <select
    class="{!labelFieldToMap ? 'text-[gray]' : ''} m-0"
    bind:value={labelFieldToMap}
  >
  ...
  {#if darwinCoreFields.includes(labelFieldToMap)}
    <a
      ...
      href={"https://dwc.tdwg.org/terms/#dwc:" + labelFieldToMap}
      target="”_blank”"><LinkIcon /></a
    >
  {/if}
  <button class="btn btn-secondary" on:click={handleAddMapping}>{$t("map", "Map")}</button>
</div>
