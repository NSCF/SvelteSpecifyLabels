...
<label class="m-auto mb-[1em] text-[grey] flex items-center">
  ...
  <a class="relative top-[4px] fill-[#617E3E] hover:fill-[#043B58]" href={"https://dwc.tdwg.org/terms/#dwc:" + labelField} target=”_blank”><LinkIcon /></a>
  {/if}
  <select class="inline text-black m-0 ml-[1em]" bind:value={$fieldMappings[$appSettings.labelType][labelField]} on:change={_ => dispatch('mapping-change')}>
    <option value="">{$t('remove', 'remove')}</option>
    {#each Object.keys(record) as datasetField}
    <option value={datasetField}>{datasetField}</option>
    {/each}
  </select>
  <button class="m-0 p-0 bg-transparent border-none mt-[8px]" on:click={handleCloseClick}><CloseIcon/></button>
</label>
