...
<div style="width: 100%;display:flex; flex-direction:column;">
  <div class="cols">
    ...
    {#if $labelSettings.showCollectionName}
      <input
        ...
        placeholder={langs["collName"][$appSettings.lang]}
      /><br />
      <br />
    {/if}
    <label style="display:inline">
    ...
    <label style="display:inline">
      <input
        type="checkbox"
        bind:checked={$labelSettings.includeStageSexOnMainLabel}
      />
    ...
    {#if $labelSettings.includeQRCode && $labelSettings.detLabel && !$labelSettings.detLabelOnly}
      <label style="display:inline">
        <input
          ...
          bind:checked={$labelSettings.qrCodeOnDetLabels}
        />
    ...
    <label style="display:inline">
      <input
        type="checkbox"
        bind:checked={$labelSettings.showStorage}
        on:change={(_) => dispatch("calc_labels")}
      />
    ...
    {#if $labelSettings.addPrintedDate}
      <input
        ...
        placeholder={langs["printer"][$appSettings.lang]}
      /><br />
      <br />
    {/if}
  </div>
    ...
    {#if $labelSettings.font in fontWeights}
      <div>
        <label for="font-weight" style="text-wrap:nowrap"
          >{langs["fontWeight"][$appSettings.lang]}</label
        >
  ...
  <p style="position:relative; margin:0; top:-1em; font-size:0.7em">
    {langs["saveSettings"][$appSettings.lang]}
  </p>
  <br />
</div>
