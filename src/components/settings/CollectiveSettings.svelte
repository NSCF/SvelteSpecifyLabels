...
<div class="flex gap-[3em]">
  ...
  <label class="inline">
    <input type="checkbox" bind:checked={$labelSettings.excludeNoCatnums} />
    {$t(
      "excludeCatNums",
      "Exclude records with no catalog or collector numbers",
    )}
  </label>
  <br />
  <label class="inline">
    <input
      type="checkbox"
      bind:checked={$labelSettings.includeCollectorInSort}
    />
