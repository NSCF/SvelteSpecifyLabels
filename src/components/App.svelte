<script>
	...
	$: $settings.showStorage, rawData.length? mapAndSortRawData() : null
  		...
  		<input type="text" name="gender" id="male" bind:value={$settings.collectionName} style="display:inline; margin:0;height:10px" placeholder="Collection name..."><br>
			...
			<div style="display:flex; gap:5px">
				<div>
					...
					<input type="number" min={minFontSize} max={maxFontSize} on:keyup={handleFontSizeKeyboardInput} on:mousewheel|preventDefault bind:value={$settings.fontSize} >
				</div>
				<div>
					<label>{langs[$settings.lang]['labelWidth']}</label>
					<div style="display:flex; align-items: baseline; ">
						<input type="number" min={minLabelWidth} max={maxLabelWidth} step=".5" on:keyup={handleLabelWidthKeyboardInput} on:mousewheel|preventDefault bind:value={$settings.labelWidth} >
